...
def research_info_search_rag(state):
    ...
    # RAG search
    rag_questions = question_rag_chain.invoke({"initial_question": initial_question, "question_category": question_category})
    ...
    return {"research_info_rag": rag_results,"rag_questions":rag_questions, "num_steps":num_steps}
...
def research_info_search_web(state):
    ...
    initial_question = state["initial_question"]
    question_category = state["question_category"]
    rag_agent_response = state["research_info_rag"] #information from rag to research
    ...
    for keyword in keywords[:1]:
        ...
        if all(isinstance(item, str) for item in temp_docs):
          ...
          web_results = "\n".join([d.get("content", "") for d in temp_docs])  # Extract 'content' if available, otherwise use empty string
    ...
    print("-----Fullsearch",full_searches)
    print(type(full_searches))
    # write_markdown_file(full_searches, "research_info")
    return {"research_info_web": full_searches, "num_steps":num_steps}
